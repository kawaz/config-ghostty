---
description: The color of the split divider
platform: all
default: ""
---

# unfocused-split-fill

**Language:** English | [日本語](../../ja/config/unfocused-split-fill.ja.md)

## Description

The color to dim the unfocused split. Unfocused splits are dimmed by rendering a semi-transparent rectangle over the split. This sets the color of that rectangle and can be used to carefully control the dimming effect.

This will default to the background color.

Specified as either hex (`#RRGGBB` or `RRGGBB`) or a named X11 color.

## Default

```conf
# https://ghostty.org/docs/config/reference#unfocused-split-fill
# The color of the split divider (default: background color)
unfocused-split-fill =
```
