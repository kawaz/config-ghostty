---
description: 変数フォント用のフォントバリエーション軸を設定する
platform: all
default: (なし)
---
...
**言語:** [English](../../en/config/font-variation.en.md) | 日本語

## 説明

変数フォント用に 1 つ以上のフォントバリエーション値を設定するための反復可能な設定。変数フォントは、単一フォント (通常は `-VF.ttf` または `-VF.otf` で終わるファイル名) で、重量、傾斜などのようなコンフィギュラブルなアクスが 1 つ以上含まれている。すべてのフォントがバリエーションをサポートしているわけではない。明示的に変数フォントとして述べているフォントのみが動作する。
